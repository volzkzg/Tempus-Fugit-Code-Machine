...
double arcSin(const double &a) { return a <= -1.0 ? -PI / 2 : (a >= 1.0 ? PI / 2 : asin(a)); }
double arcCos(const double &a) { return a <= -1.0 ? PI : (a >= 1.0 ? 0 : acos(a)); }
struct point {
...
vector<pair<point, point> > tanCC(const circle &cir1, const circle &cir2) { 
    ...
    if (sign(r1 - r2) == 0) {
        p = c2 - c1; p = (p * (r1 / p.len())).rot90();
        list.push_back(make_pair(c1 + p, c2 + p)); list.push_back(make_pair(c1 - p, c2 - p));
    } else {
        p = (c2 * r1 - c1 * r2) / (r1 - r2);
        s1 = cir1.tanCP(p, a1, b1); s2 = cir2.tanCP(p, a2, b2);
        if (s1 >= 1 && s2 >= 1) {
            list.push_back(make_pair(a1, a2)); list.push_back(make_pair(b1, b2)); } }
    p = (c1 * r2 + c2 * r1) / (r1 + r2);
    s1 = cir1.tanCP(p, a1, b1); s2 = cir2.tanCP(p, a2, b2);
    if (s1 >= 1 && s2 >= 1) {
        list.push_back(make_pair(a1, a2)); list.push_back(make_pair(b1, b2)); }
